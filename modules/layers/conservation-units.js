...
const HALO_LAYER_ID='lyr-conservation-units-halo';
...
export async function register(map){
  ...
  if (!map.getLayer(FILL_LAYER_ID)) map.addLayer({id:FILL_LAYER_ID,type:'fill',source:SOURCE_ID,paint:{'fill-color':LAYER.color,'fill-opacity':0.30},layout:{visibility}});
  ...
  if (!map.getLayer(LINE_LAYER_ID)) map.addLayer({id:LINE_LAYER_ID,type:'line',source:SOURCE_ID,paint:{'line-color':'#0F3D2E','line-width':1.8,'line-opacity':0.95},layout:{visibility}});
}
